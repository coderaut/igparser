...
- **`fetch_post()` error contract:** it only ever raises `RuntimeError` (auth/timeout/HTTP error, empty dataset, or total media failure). `_extract_shortcode` raises `ValueError` for a non-post URL, but `fetch_post` wraps that into a `RuntimeError` so every caller (CLI catches `RuntimeError`, app catches broadly) handles one exception type. Don't reintroduce a bare `ValueError` escape.
- **Carousel slide MIME (deferred minor):** `_download_media` always names slides `slide_NN.jpg`, and `image_reader.py` infers the data-URL MIME from the file suffix. If Instagram's CDN serves WebP/PNG, the bytes get mislabeled `image/jpeg` — usually harmless (graph CDN serves JPEG; verified fine on the 2026-06-27 live carousel), but if OCR ever breaks on a carousel, derive the extension from the response `Content-Type` instead of hardcoding `.jpg`.
- **Apify response field names** the parser depends on (verified against live posts 2026-06-27): `videoUrl`, `childPosts[].displayUrl`/`childPosts[].videoUrl` (video children skipped), `displayUrl`, `caption`, `url`, `ownerUsername`, `ownerFullName`, `timestamp`, `type`, `shortCode`. If Apify renames any, `_parse_item` in `fetcher.py` and the fixtures under `tests/fixtures/` both need updating.
- **`cookies/` stays git-ignored** even though it's unused — a real `cookies/instagram.txt` session file may linger on the host from the pre-Apify era; the `.gitignore` entry prevents `git add -A` from ever committing that secret.
